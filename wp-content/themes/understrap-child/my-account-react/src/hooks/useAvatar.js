...
/**
 * Custom hook to manage avatar state across the application
 * Returns avatar type, selected emoji, uploaded URL, and loading state
 */
export const useAvatar = () => {
      ...
      try {
        ...
        if (response.success && response.preferences) {
          ...
          if (prefs.avatarType === 'emoji' && prefs.avatarEmoji) {
            // Find the matching emoji option from avatarOptions
  ...
  const refetchAvatar = async () => {
    ...
    try {
      ...
      console.error('Failed to refetch avatar preferences:', error);
    } finally {
      setLoading(false);
    }
  };
